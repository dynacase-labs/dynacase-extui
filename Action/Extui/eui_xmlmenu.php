...
function eui_getxmlmenu($docid, $menuxml, $folderid = 0)
{
    ...
    if (!$lay->file) {
        ...
        if ($err != "") $out["error"] = $err;
            ...
            $gen = $lay->gen();
            $gen = str_replace(array(
                "<ext ",
                "</ext>"
            ) , array(
                "<script ",
                "</script>"
            ) , $gen); // to delete - need for compliant with old spec
            $sepk = 0;
...
function parseItemMenu($im, & $doc = null, &$folder = null)
{
    ...
    return array(
        ...
        "backgroundColor" => current($im->backgroundColor) ,
        "icon" => $im->icon ? @current($im->icon->attributes()->src) : false,
        "confirm" => (current($im->confirm)) ? array(
            "label" => current($im->confirm->label) ,
            "continue" => current($im->confirm->continue) ,
            "cancel" => current($im->confirm->cancel)
        ) : false,
        "script" => current($im->script) ? array(
            "file" => current($im->script->attributes()->file) ,
            "class" => current($im->script->attributes()->class) ,
            "parameters" => @current($im->script->attributes()->parameters)
        ) : false
    );
}
